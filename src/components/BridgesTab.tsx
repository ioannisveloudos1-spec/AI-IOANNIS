...
const PRESET_PAIRS = [
  ...
  { a: "ΒΕΛΟΣ", b: "ΟΥΔΟΣ", desc: "305 + 744 = 1049 (Μυστική Πύλη 1049)" },
...
const PRESET_CROSS_PAIRS = [
  ...
  { greek: "ΒΕΛΟΥΔΟΣ", english: "SECRET", targetVal: 781, desc: "Ιωνική: 781" },
];
...
export const BridgesTab: React.FC<BridgesTabProps> = ({
  ...
  const [targetSumInput, setTargetSumInput] = useState<string>("");
  const [targetWordCount, setTargetWordCount] = useState<number>(4);
  const [generatorStrategy, setGeneratorStrategy] = useState<GenerationStrategy>("grammatical");
  const [generatorSource, setGeneratorSource] = useState<VocabularySource>("all");
  ...
  }, [targetNumParsed, targetWordCount, generatorStrategy, generatorSource, savedItems]);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-5 rounded-2xl bg-[#14120e] border border-[#2d241a] shadow-md space-y-3">
              ...
              {showPairLetterBreakdown && (
                <div className="flex flex-wrap gap-1 pt-1 animate-fadeIn">
          ...
          {/* Toggle Button for Pair Letter Breakdown */}
          <div className="flex justify-end">
            <button
              type="button"
              onClick={() => setShowPairLetterBreakdown((prev) => !prev)}
              className="px-3 py-1.5 rounded-lg bg-[#16130f] border border-[#2d241a] hover:border-[#c89b3c]/50 text-[#a69680] hover:text-[#f5ecd8] text-xs font-serif flex items-center gap-1.5 cursor-pointer transition-all"
            >
              <Layers className="w-3.5 h-3.5 text-[#c89b3c]" />
              <span>{showPairLetterBreakdown ? "Απόκρυψη ανάλυσης γραμμάτων" : "Προβολή ανάλυσης γραμμάτων"}</span>
              {showPairLetterBreakdown ? (
      ...
      {activeMode === "sentence_builder" && (
        <div className="space-y-6">
          {/* Controls Bar */}
          <div className="p-5 rounded-2xl bg-[#14120e] border border-[#2d241a] shadow-lg space-y-5">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <Wand2 className="w-5 h-5 text-emerald-400" />
                  <h3 className="text-base font-serif font-bold text-[#f5ecd8]">
                    Γεννήτρια Ισόψηφων Προτάσεων με Γραμματικό & Συντακτικό Νόημα
                  </h3>
                </div>
                <p className="text-xs text-[#a69680]">
                  Συνθέτει έγκυρες συντακτικές προτάσεις (Άρθρο + Υποκείμενο + Ρήμα + Αντικείμενο) που αθροίζουν επακριβώς στον αριθμό-στόχο.
                </p>
              </div>

              {/* Quick Presets */}
              <div className="flex items-center gap-1.5 flex-wrap">
                {[666, 781, 888, 1000, 1119, 1480, 1900, 2024].map((num) => (
                  <button
                    key={num}
            ...
            {/* Inputs: Target Sum & Strategy / Source / Word Count */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-3 border-t border-[#261e14]">
              {/* Target Sum */}
              <div>
              ...
              <div>
                <label className="text-xs font-serif text-[#c89b3c] font-bold block mb-1">
                  Δομή & Λογική Σύνθεσης
                </label>
                <div className="flex bg-[#1b1712] p-1 rounded-xl border border-[#382b1d]">
                  <button
                    onClick={() => setGeneratorStrategy("grammatical")}
                    className={`flex-1 py-2 px-2 rounded-lg text-xs font-serif transition-colors cursor-pointer flex items-center justify-center gap-1.5 ${
                      generatorStrategy === "grammatical"
                        ? "bg-[#282116] text-[#e6c670] font-bold border border-[#c89b3c]/40"
                        : "text-[#8c7e6c] hover:text-[#d6c7b2]"
                    }`}
                  >
                    <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
                    <span>Γραμματική Σύνταξη</span>
                  </button>
                  <button
                    onClick={() => setGeneratorStrategy("free")}
                    className={`flex-1 py-2 px-2 rounded-lg text-xs font-serif transition-colors cursor-pointer flex items-center justify-center gap-1.5 ${
                      generatorStrategy === "free"
                        ? "bg-[#282116] text-[#e6c670] font-bold border border-[#c89b3c]/40"
                        : "text-[#8c7e6c] hover:text-[#d6c7b2]"
                    }`}
                  >
                    <Sliders className="w-3.5 h-3.5 text-[#a69680]" />
                    <span>Ελεύθερη Σύνθεση</span>
                  </button>
                </div>
              </div>
              ...
              <div>
                <label className="text-xs font-serif text-[#c89b3c] font-bold block mb-1">
                  Πλήθος Λέξεων στη Φράση
                </label>
                <div className="flex bg-[#1b1712] p-1 rounded-xl border border-[#382b1d]">
                  {[
                    { count: 2, label: "2" },
                    { count: 3, label: "3" },
                    { count: 4, label: "4 (Άρθρο+Υπ+Ρήμα+Αντ)" },
                    { count: 5, label: "5" },
                  ].map((opt) => (
                    <button
                      key={opt.count}
                      onClick={() => setTargetWordCount(opt.count)}
                      className={`flex-1 py-2 rounded-lg text-xs font-serif transition-colors cursor-pointer ${
                        targetWordCount === opt.count
                          ? "bg-[#282116] text-[#e6c670] font-bold border border-[#c89b3c]/40"
                          : "text-[#8c7e6c] hover:text-[#d6c7b2]"
                      }`}
                      title={`${opt.count} Λέξεις`}
                    >
                      {opt.label}
                    </button>
                  ))}
            ...
            {/* Source Selector Bar */}
            <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-[#261e14] text-xs font-serif">
              <span className="text-[#a69680]">Πηγή Λεξιλογίου Αναζήτησης:</span>
              <div className="flex items-center gap-2">
                {[
                  { key: "all" as VocabularySource, label: `Όλες οι Πηγές (Αρχείο + Κλασικό)` },
                  { key: "archive_only" as VocabularySource, label: `Μόνο Αρχείο Χρήστη (${savedItems.length})` },
                  { key: "classical_only" as VocabularySource, label: `Μόνο Κλασικό / Φιλοσοφικό` },
                ].map((src) => (
                  <button
                    key={src.key}
                    onClick={() => setGeneratorSource(src.key)}
                    className={`px-3 py-1 rounded-lg transition-all cursor-pointer ${
                      generatorSource === src.key
                        ? "bg-[#241d15] text-[#e6c670] font-bold border border-[#c89b3c]/50 shadow-sm"
                        : "bg-[#161310] text-[#736655] hover:text-[#c4b5a0] border border-[#2d241a]"
                    }`}
                  >
                    {src.label}
                  </button>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between text-xs font-serif text-[#8c7e6c]">
              <span>
                Εντοπίστηκαν <strong>{generatedSentences.length}</strong> συνδυασμοί {targetWordCount} λέξεων για το άθροισμα <strong>{targetNumParsed}</strong>
              </span>
              {generatorStrategy === "grammatical" && (
                <span className="text-emerald-400/90 flex items-center gap-1">
                  <CheckCircle2 className="w-3.5 h-3.5" />
                  Φίλτρο Γραμματικής & Συντακτικής Συνοχής Ενεργό
                </span>
              )}
            </div>
            ...
            {generatedSentences.length === 0 ? (
              <div className="p-10 rounded-2xl bg-[#14120e] border border-[#2d241a] text-center space-y-2">
                ...
                <p className="text-sm font-serif text-[#a69680]">
                  Δεν βρέθηκε ακριβής συνδυασμός {targetWordCount} λέξεων με άθροισμα {targetNumParsed} στις επιλεγμένες ρυθμίσεις.
                </p>
                <p className="text-xs text-[#736452]">
                  Δοκιμάστε να επιλέξετε «Ελεύθερη Σύνθεση», διαφορετικό πλήθος λέξεων ή προσθέστε νέες λέξεις στο Αρχείο σας!
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3.5">
                {generatedSentences.map((match) => (
                  <div
                    key={match.id}
                    className="p-4 rounded-xl bg-[#181410] border border-[#382b1d] hover:border-[#c89b3c] transition-all space-y-3 shadow-md relative group"
                  >
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-xs font-mono font-bold text-emerald-400 bg-emerald-950/50 px-2 py-0.5 rounded border border-emerald-800/80">
                        Σύνολο = {match.totalSum}
                      </span>
                      <span className="text-[11px] font-serif text-[#c89b3c] bg-[#221b13] px-2 py-0.5 rounded border border-[#3e2e1e]">
                        {match.structureLabel}
                      </span>
                    </div>

                    <div className="text-base sm:text-lg font-serif font-bold text-[#f5ecd8] leading-snug tracking-wide">
                      «{match.fullSentence}»
                    </div>

                    {/* Word Grammatical Breakdown Badges */}
                    <div className="flex flex-wrap gap-1.5 text-xs font-mono">
                      {match.words.map((w, wIdx) => {
                        let posBg = "bg-[#221b13] text-[#f5ecd8] border-[#3e2f1e]";
                        if (w.pos === "article") posBg = "bg-cyan-950/50 text-cyan-300 border-cyan-800/60";
                        if (w.pos === "subject") posBg = "bg-amber-950/50 text-amber-300 border-amber-800/60";
                        if (w.pos === "verb") posBg = "bg-emerald-950/50 text-emerald-300 border-emerald-800/60";
                        if (w.pos === "object") posBg = "bg-purple-950/50 text-purple-300 border-purple-800/60";
                        if (w.pos === "adjective") posBg = "bg-yellow-950/50 text-yellow-300 border-yellow-800/60";
                        if (w.pos === "preposition") posBg = "bg-orange-950/50 text-orange-300 border-orange-800/60";

                        return (
                          <div
                            key={wIdx}
                            className={`px-2 py-1 rounded-lg border text-[11px] flex items-center gap-1.5 ${posBg}`}
                          >
                            <span className="font-bold">{w.text}</span>
                            <span className="text-[10px] opacity-75 font-mono">({w.value})</span>
                            <span className="text-[9px] uppercase px-1 rounded bg-black/40 font-serif opacity-90">
                              {w.roleLabel || w.pos}
                            </span>
                          </div>
                        );
                    ...
                    <div className="pt-2 border-t border-[#261e14] flex items-center justify-between">
                      <button
                        onClick={() =>
                          onOpenAiModal(
                            `Ερμηνεία Ισόψηφης Πρότασης: ${match.fullSentence} (Άθροισμα: ${match.totalSum}, Δομή: ${match.structureLabel})`,
                            match.totalSum,
                      ...
                      <button
                        onClick={() => {
                          onSaveItem({
                            ...
                            notes: `Σύνθεση ${match.wordCount} λέξεων (${match.structureLabel}) με άθροισμα ${match.totalSum} (${match.words.map((w) => `${w.text}:${w.value}`).join(" + ")})`,
                          });
