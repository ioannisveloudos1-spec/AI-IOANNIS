    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      {activeSubTab === "compare" && compareItems.length > 0 && (
        <div className="space-y-6 animate-in fade-in duration-300">
          <div className="p-4 sm:p-5 rounded-2xl bg-[#161310] border border-[#c89b3c]/40 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-lg shadow-black/30">
            <div>
              <h3 className="text-base font-serif font-bold text-[#f5ecd8] flex items-center gap-2">
                <GitCompare className="w-5 h-5 text-[#c89b3c]" />
                <span>Αντιπαραβολή & Σύγκριση {compareItems.length} Επιλεγμένων Λεξαρίθμων</span>
              </h3>
              <p className="text-xs text-[#a69680] mt-0.5">
                Άμεση αριθμοσοφική και μαθηματική αντιπαραβολή, ισοψηφικές διαφορές (Δ), κοινοί πυθμένες και αναλογίες.
              </p>
            </div>
            <div className="flex items-center gap-2">
              <button
                ...
                onClick={() =>
                  onOpenAiModal(
                    compareItems.map((i) => `${i.text} (${i.value})`).join(" vs "),
                    compareItems.reduce((acc, curr) => acc + curr.value, 0),
                    compareItems.map((i) => i.text)
                  )
                }
                className="px-3 py-1.5 rounded-xl bg-[#282015] hover:bg-[#382c1d] border border-[#c89b3c]/60 text-xs font-serif text-[#e6c670] flex items-center gap-1.5 transition-all shadow-sm"
              >
                <Sparkles className="w-3.5 h-3.5 text-[#c89b3c]" />
                <span>Συνδυαστική AI Ανάλυση</span>
              </button>
              <button
                type="button"
                onClick={() => setCompareItemIds([])}
                className="px-3 py-1.5 rounded-xl bg-[#201812] hover:bg-red-950/50 border border-[#3a2c1f] hover:border-red-600/50 text-xs font-serif text-[#a69680] hover:text-red-300 transition-all"
              >
                Εκκαθάριση
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {compareItems.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="p-4 rounded-2xl bg-[#181410] border border-[#3e3223] space-y-4 relative shadow-md shadow-black/20"
                >
                  <button
                    type="button"
                    onClick={() => toggleCompare(item.id)}
                    className="absolute top-3 right-3 p-1 rounded-md bg-[#221a12] text-[#8c7e6c] hover:text-red-400 border border-[#3a2c1e] hover:border-red-700/50 transition-colors"
                    title="Αφαίρεση από τη σύγκριση"
                  >
                    <X className="w-3.5 h-3.5" />
                  </button>

                  <div className="border-b border-[#29221a] pb-2 pr-6">
                    <span className="text-base sm:text-lg font-serif font-bold text-[#f5ecd8] block truncate" title={item.text}>
                      {item.text}
                    </span>
                    <div className="flex items-baseline gap-2 mt-1">
                      ...
                      <span className="text-xs font-serif font-bold text-[#c89b3c] bg-[#241d15] px-2 py-0.5 rounded border border-[#443522]">
                        {numberToGreekNumeral(item.value)}
                      </span>
                    </div>
                  </div>

                  <div className="space-y-2 text-xs font-mono text-[#a69680]">
                    <div className="flex justify-between py-0.5 border-b border-[#241c15]">
                      <span>Πυθμένας (Ρίζα):</span>
                      <strong className="text-[#e6c670]">{mathProps.pythmen}</strong>
                    </div>
                    <div className="flex justify-between py-0.5 border-b border-[#241c15]">
                      <span>Τύπος Αριθμού:</span>
                      <span className="text-[#f5ecd8]">{mathProps.isEven ? "Άρτιος (Ζυγός)" : "Περιττός (Μονός)"}</span>
                    </div>
                    <div className="flex justify-between py-0.5 border-b border-[#241c15]">
                      <span>Πρώτος Αριθμός:</span>
                      <span className={mathProps.isPrime ? "text-emerald-400 font-bold" : "text-[#736655]"}>
                        {mathProps.isPrime ? "Ναι (Prime)" : "Όχι"}
                      </span>
                    </div>
                    <div className="flex justify-between py-0.5 border-b border-[#241c15]">
                      <span>Τρίγωνος Αριθμός:</span>
                      <span className={mathProps.isTriangular ? "text-amber-400 font-bold" : "text-[#736655]"}>
                    ...
                    {item.category && (
                      <div className="flex justify-between py-0.5 border-b border-[#241c15]">
                        <span>Κατηγορία:</span>
                        <span className="text-[#c89b3c] font-serif text-[11px] truncate max-w-[120px]">{item.category}</span>
                      </div>
                    )}
                  </div>
          ...
          {/* Pairwise Mathematical Relationships Table (if 2 or more items) */}
          {compareItems.length >= 2 && (
            <div className="p-4 sm:p-5 rounded-2xl bg-[#15120f] border border-[#3e3223] space-y-4">
              <h4 className="text-sm font-serif font-bold text-[#e6c670] flex items-center gap-2">
                <ArrowLeftRight className="w-4 h-4 text-[#c89b3c]" />
                <span>Μαθηματικές Σχέσεις & Ισοψηφικές Διαφορές Μεταξύ των Λέξεων</span>
              </h4>

              <div className="overflow-x-auto">
                <table className="w-full text-xs font-serif text-left border-collapse">
                  <thead>
                    <tr className="border-b border-[#2d241b] text-[#a69680]">
                      <th className="py-2.5 px-3">Ζεύγος Λέξεων</th>
                      <th className="py-2.5 px-3">Λεξάριθμοι</th>
                      <th className="py-2.5 px-3">Διαφορά (Δ)</th>
                      <th className="py-2.5 px-3">Άθροισμα (Σ)</th>
                      <th className="py-2.5 px-3">Ίδιος Πυθμένας;</th>
                      <th className="py-2.5 px-3">Κοινοί Διαιρέτες</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#241c15]">
                    {compareItems.flatMap((itemA, i) =>
                      compareItems.slice(i + 1).map((itemB) => {
                        const diff = Math.abs(itemA.value - itemB.value);
                        const sum = itemA.value + itemB.value;
                        const mathA = getMathematicalProperties(itemA.value);
                        const mathB = getMathematicalProperties(itemB.value);
                        const samePythmen = mathA.pythmen === mathB.pythmen;

                        // Calculate greatest common divisor (GCD)
                        const gcd = (a: number, b: number): number => (b === 0 ? a : gcd(b, a % b));
                        const commonGcd = gcd(itemA.value, itemB.value);

                        return (
                          <tr key={`${itemA.id}-${itemB.id}`} className="hover:bg-[#1a1612] transition-colors">
                            <td className="py-3 px-3 font-bold text-[#f5ecd8]">
                              {itemA.text} <span className="text-[#c89b3c] font-normal">↔</span> {itemB.text}
                            </td>
                            <td className="py-3 px-3 font-mono text-[#e6c670]">
                              {itemA.value} ↔ {itemB.value}
                            </td>
                            <td className="py-3 px-3 font-mono font-bold text-amber-300">
                              {diff === 0 ? "0 (Απόλυτη Ισοψηφία!)" : `Δ = ${diff}`}
                            </td>
                            <td className="py-3 px-3 font-mono text-[#f5ecd8]">
                              {sum.toLocaleString("el-GR")}
                            </td>
                            <td className="py-3 px-3">
                              {samePythmen ? (
                                ...
                                <span className="text-[#736655] text-[11px]">
                                  Όχι ({mathA.pythmen} vs {mathB.pythmen})
                                </span>
                              )}
                            </td>
                            <td className="py-3 px-3 font-mono text-[#a69680]">
                              {commonGcd > 1 ? (
                                <span className="text-[#e6c670]">ΜΚΔ: {commonGcd}</span>
                              ) : (
                                <span className="text-[#736655]">Πρώτοι μεταξύ τους</span>
                              )}
                            </td>
                          </tr>
                        );
      ...
      {/* Floating Bottom Comparison Dock (Visible across all subtabs when items are chosen) */}
      {compareItemIds.length > 0 && activeSubTab !== "compare" && (
        <div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-40 w-[95%] max-w-2xl p-3 sm:p-4 rounded-2xl bg-[#181410]/95 backdrop-blur-md border-2 border-[#c89b3c] shadow-2xl shadow-black/80 animate-in slide-in-from-bottom-5 duration-300">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <GitCompare className="w-4 h-4 text-[#c89b3c] animate-pulse" />
                <span className="text-xs font-serif font-bold text-[#f5ecd8]">
                  Επιλεγμένα για Σύγκριση ({compareItemIds.length}/4)
                </span>
              </div>
              <div className="flex flex-wrap gap-1.5 max-h-16 overflow-y-auto">
                {compareItems.map((item) => (
                  <span
                    key={item.id}
                    className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-[#261f17] border border-[#c89b3c]/50 text-xs font-serif text-[#f5ecd8]"
                  >
                    <strong className="text-[#e6c670]">{item.text}</strong>
                    <span className="text-[10px] font-mono text-[#c89b3c]">({item.value})</span>
                    <button
                      type="button"
                      onClick={() => toggleCompare(item.id)}
                      className="text-[#a69680] hover:text-red-400 ml-0.5"
                    >
                      <X className="w-3 h-3" />
                    </button>
                  </span>
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                className="px-4 py-2 rounded-xl bg-gradient-to-r from-[#c89b3c] to-[#e6c670] hover:brightness-110 text-[#120f0c] text-xs font-serif font-bold transition-all shadow-md shadow-[#c89b3c]/20 flex items-center gap-1.5 cursor-pointer"
              >
                <GitCompare className="w-4 h-4" />
                <span>Προβολή Πίνακα ({compareItemIds.length})</span>
              </button>
              <button
                type="button"
                onClick={() => setCompareItemIds([])}
                className="p-2 rounded-xl bg-[#221a13] hover:bg-red-950/60 border border-[#3e2f20] text-[#a69680] hover:text-red-300 transition-colors"
                title="Ακύρωση σύγκρισης"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
