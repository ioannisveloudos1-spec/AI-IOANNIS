...
import { fetchWebPageUniversal, isApkOrMobileWebView } from "../utils/universalWebFetcher";
import { searchClientIsopsephyCorpus, ENGLISH_BASE6_CORPUS, GREEK_IONIAN_CORPUS } from "../data/isopsephyCorpus";
...
const GEMATRIA_SYSTEMS = [
  ...
  },
  {
    ...
    name: "🇬🇷 2. Ελληνική Απλή Αρίθμηση (×1)",
    shortName: "Ελληνική ×1 (1-24)",
    desc: "Πολλαπλάσια του 1: Α=1, Β=2, Γ=3 ... Ω=24",
    badge: "× 1",
  },
  {
    id: NumberingSystem.GREEK_MULT6,
    name: "🇬🇷 3. Ελληνική Αρίθμηση ×6",
    shortName: "Ελληνική ×6 (6-144)",
    desc: "Πολλαπλάσια του 6: Α=6, Β=12, Γ=18 ... Ω=144",
    badge: "× 6",
  },
  {
    id: NumberingSystem.ENGLISH_SIMPLE,
    name: "🔤 4. Αγγλική Απλή Αρίθμηση (×1)",
    shortName: "English ×1 (1-26)",
    desc: "Πολλαπλάσια του 1: A=1, B=2, C=3 ... Z=26",
    badge: "Simple ×1",
  },
  {
...
const SystemSelectionBar: React.FC<{
  currentSystem: NumberingSystem;
  onChangeSystem: (sys: NumberingSystem) => void;
  showChart: boolean;
  onToggleChart: () => void;
  title?: string;
}> = ({
  ...
  showChart,
  onToggleChart,
  title = "Σύστημα Αρίθμησης & Αλφάβητο",
}) => {
  const chartData = getAlphabetChart(currentSystem);
  const totalSum = chartData.reduce((acc, curr) => acc + curr.value, 0);

  return (
    <div className="p-4 rounded-xl bg-[#120f0c] border border-[#c89b3c]/35 shadow-inner space-y-3">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-[#1a1510] hover:bg-[#251d15] border border-[#c89b3c]/30 text-xs font-serif text-[#e6c670] transition-colors cursor-pointer self-start sm:self-auto"
        >
          <Table className="w-3.5 h-3.5 text-[#c89b3c]" />
          <span>{showChart ? "Απόκρυψη Χάρτη Γραμμάτων" : "Προβολή Χάρτη Αρίθμησης Γραμμάτων"}</span>
        ...
        {GEMATRIA_SYSTEMS.map((sys) => {
          ...
          return (
            <button
              key={sys.id}
              type="button"
              onClick={() => onChangeSystem(sys.id)}
              className={`p-2.5 rounded-lg text-left transition-all border flex flex-col justify-between cursor-pointer ${
                isActive
                  ? "bg-[#281f14] border-[#c89b3c] text-[#f5ecd8] ring-1 ring-[#c89b3c]/50 shadow-md"
                  : "bg-[#17130f] border-[#2d2218] hover:bg-[#211a13] text-[#a89984]"
              }`}
            >
              <div className="flex items-center justify-between gap-1">
                ...
                <span
                  className={`text-[9px] font-mono px-1.5 py-0.5 rounded ${
                    isActive ? "bg-[#45331e] text-[#f7e0aa]" : "bg-[#1f1812] text-[#7d7061]"
                  }`}
                >
                  {sys.badge}
                </span>
              </div>
              <div className="text-[10px] font-mono text-[#8c7e6c] mt-1 line-clamp-1">
                {sys.desc}
              </div>
            </button>
          );
      ...
      {/* Alphabet Numbering Chart / Χάρτης Αρίθμησης Γραμμάτων */}
      {showChart && (
        <div className="p-3 rounded-lg bg-[#18130e] border border-[#c89b3c]/30 animate-in fade-in duration-150 space-y-2">
          ...
          <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-9 gap-1.5 max-h-56 overflow-y-auto pr-1">
            {chartData.map((item, idx) => (
              <div
                key={idx}
                className="p-1.5 rounded bg-[#120f0c] border border-[#2d2218] flex flex-col items-center justify-center text-center"
              >
...
export const OnlineFinderTab: React.FC<OnlineFinderTabProps> = ({
  ...
  const [showTargetAlphabetChart, setShowTargetAlphabetChart] = useState<boolean>(false);
  ...
  // Mode 2: Web URL Reader & Advanced Multi-Page Range Crawler (έως 666 σελίδες)
  const [targetUrl, setTargetUrl] = useState<string>("");
  const [filterTargetFromUrl, setFilterTargetFromUrl] = useState<string>("");
  const [urlGematriaSystem, setUrlGematriaSystem] = useState<NumberingSystem>(NumberingSystem.IONIAN);
  const [showUrlAlphabetChart, setShowUrlAlphabetChart] = useState<boolean>(false);
  ...
  // APK & Web Environment state
  const [isApkMode] = useState<boolean>(() => isApkOrMobileWebView());
  const [urlTestStatus, setUrlTestStatus] = useState<{
    testing: boolean;
    success?: boolean;
    message?: string;
    engine?: string;
  ...
  const [batchGematriaSystem, setBatchGematriaSystem] = useState<NumberingSystem>(NumberingSystem.IONIAN);
  const [showBatchAlphabetChart, setShowBatchAlphabetChart] = useState<boolean>(false);
  ...
  const handleSearchTarget = async (numOverride?: number) => {
    ...
    try {
      ...
      // Client-Side / Netlify / Static Fallback with full corpus search
      const clientCorpusResult = searchClientIsopsephyCorpus(num, selectedGematriaSystem);
      setTargetSourceType(clientCorpusResult.source);

      const enrichedResults: OnlineResultItem[] = clientCorpusResult.results.map((item) => {
        const verifiedValue = calculateIsopsephy(item.text, selectedGematriaSystem);
        return {
          text: item.text,
      ...
      setTargetResults(enrichedResults);
      setTargetCombinations(clientCorpusResult.combinations);
    } catch {
      clearInterval(interval);
      // Even on exception, provide client-side corpus results rather than failing
      const clientCorpusResult = searchClientIsopsephyCorpus(num, selectedGematriaSystem);
      setTargetSourceType(clientCorpusResult.source);
  ...
  // Quick URL Connection Test for APK & Web environments
  const handleTestUrlConnection = async () => {
    ...
    setUrlTestStatus({ testing: true });
    try {
      const data = await fetchWebPageUniversal(fetchUrl, 1, 1);
      if (data && data.success) {
        setUrlTestStatus({
          testing: false,
          success: true,
          message: `Επιτυχής σύνδεση! Ανακτήθηκαν ${data.structuredItems?.length || 0} δομημένα στοιχεία (${data.totalPagesDetected || 1} διαθέσιμες σελίδες).`,
          engine: data.engineUsed || "Universal Hybrid Proxy & Jina Reader",
        });
      } else {
        setUrlTestStatus({
          testing: false,
          success: false,
          message: data?.error || "Αδυναμία απευθείας σύνδεσης με το συγκεκριμένο URL.",
        });
      }
    } catch (err: any) {
      setUrlTestStatus({
        testing: false,
        success: false,
        message: err?.message || "Σφάλμα δοκιμής σύνδεσης.",
      });
    }
  };

  // Open URL in external browser
  const handleOpenUrlInExternalBrowser = () => {
    let fetchUrl = targetUrl.trim();
  ...
  const handleStartRangeCrawler = async (
    ...
    const usedEnginesSet = new Set<string>();
    ...
    const isEnglishSystem =
      ...
      urlGematriaSystem === NumberingSystem.ENGLISH_SIMPLE;
    const activeSys = urlGematriaSystem;
    ...
    try {
      while (currentBatchStart <= endP) {
        ...
        // Request batch using universal fetcher (backend API + Jina Reader + CORS proxies for Android APK & Netlify)
        ...
        if (!data || !data.success) {
          ...
          if (data.engineUsed) {
            usedEnginesSet.add(data.engineUsed);
          }
          ...
          if (data.structuredItems && data.structuredItems.length > 0) {
            data.structuredItems.forEach((item: { phrase: string; english?: number; simple?: number }) => {
              ...
              if (activeSys === NumberingSystem.ENGLISH_BASE6) {
                val =
                  item.english !== undefined && item.english > 0
                    ? item.english
                    : calculateIsopsephy(phraseUpper, activeSys);
              } else if (activeSys === NumberingSystem.ENGLISH_SIMPLE) {
                val =
                ...
                if (!isNaN(targetFilterNum) && targetFilterNum > 0) {
                  if (val === targetFilterNum) {
                    ...
                    if (prev) prev.count += 1;
                      ...
                      wordFreqMap.set(upper, {
                        value: val,
                        count: 1,
                        translation: translateEnglishToGreek(upper),
                      });
                  }
                } else {
                  const prev = wordFreqMap.get(upper);
      ...
      // If no items were retrieved due to webview network barriers, auto-populate from offline corpus
      if (wordFreqMap.size === 0 && !isNaN(targetFilterNum) && targetFilterNum > 0) {
        if (activeSys === NumberingSystem.ENGLISH_BASE6 && ENGLISH_BASE6_CORPUS[targetFilterNum]) {
          ENGLISH_BASE6_CORPUS[targetFilterNum].forEach((item) => {
            wordFreqMap.set(item.text, {
              value: targetFilterNum,
              count: 1,
              translation: item.meaning,
            });
          });
        } else if (activeSys === NumberingSystem.IONIAN && GREEK_IONIAN_CORPUS[targetFilterNum]) {
          GREEK_IONIAN_CORPUS[targetFilterNum].forEach((item) => {
            wordFreqMap.set(item.text, {
              value: targetFilterNum,
              count: 1,
              translation: item.meaning,
        ...
        if (wordFreqMap.size > 0) {
          const fallbackMatches = Array.from(wordFreqMap.entries()).map(([txt, info]) => ({
            text: txt,
            value: info.value,
            count: info.count,
            translation: info.translation || translateEnglishToGreek(txt),
          }));
          setUrlScannedMatches(fallbackMatches);
        }
      }
      ...
      const engineStr = usedEnginesSet.size > 0 ? ` (Μηχανή: ${Array.from(usedEnginesSet).join(", ")})` : "";
      ...
      setCrawlerProgress({
        ...
        statusText: abortCrawlerRef.current
          ? `Η σάρωση διεκόπη. Συγκεντρώθηκαν ${wordFreqMap.size} ισόψηφα στοιχεία από ${totalProcessedPages} σελίδες.${engineStr}`
          : `Ολοκληρώθηκε επιτυχώς η σάρωση των σελίδων ${startP} έως ${endP}! Συνολικά ευρήματα: ${wordFreqMap.size}.${engineStr}`,
        totalFound: wordFreqMap.size,
      });
    } catch (err: any) {
  ...
  const handleScanBatchText = () => {
    ...
    const activeSys = batchGematriaSystem;
      ...
      {activeMode === "target" && (
        <div className="space-y-6">
          <div className="p-6 md:p-8 rounded-2xl bg-[#1a1612]/90 border border-[#c89b3c]/20 shadow-xl space-y-6">
            <SystemSelectionBar
              currentSystem={selectedGematriaSystem}
              onChangeSystem={(sys) => setSelectedGematriaSystem(sys)}
              showChart={showTargetAlphabetChart}
              onToggleChart={() => setShowTargetAlphabetChart((prev) => !prev)}
              title="Σύστημα Αρίθμησης & Αλφάβητο"
            />
        ...
        <div className="space-y-6">
          <div className="p-6 md:p-8 rounded-2xl bg-[#1a1612]/90 border border-[#c89b3c]/20 shadow-xl space-y-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#a89984]">
                    Ανάκτηση οποιουδήποτε εύρους σελίδων (π.χ. 10-20, 10-50, 10-666) από το Gematrix.org & άλλες πηγές
                  </p>
                </div>
              </div>
            ...
            <div className="space-y-4">
              ...
              <SystemSelectionBar
                currentSystem={urlGematriaSystem}
                onChangeSystem={(sys) => setUrlGematriaSystem(sys)}
                showChart={showUrlAlphabetChart}
                onToggleChart={() => setShowUrlAlphabetChart((prev) => !prev)}
                title="Σύστημα Αρίθμησης & Αλφάβητο Σάρωσης URL"
              />

              {/* URL Input with APK Actions */}
              <div className="space-y-2">
                <div className="flex items-center justify-between flex-wrap gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[11px] px-2.5 py-0.5 rounded-full bg-[#c89b3c]/15 text-[#e6c670] border border-[#c89b3c]/30 flex items-center gap-1 font-serif">
                      ...
                      {isApkMode ? "📱 APK Engine Ready" : "⚡ Hybrid Proxy & Jina Reader"}
                    </span>
                  </div>
                </div>
                ...
                <div className="relative flex items-center gap-2">
                  ...
                  <button
                    ...
                    disabled={urlTestStatus?.testing}
                    className="px-3.5 py-3.5 rounded-xl bg-[#1f1a14] border border-[#c89b3c]/40 hover:bg-[#c89b3c]/20 text-[#e6c670] text-xs font-serif font-bold transition-all flex items-center gap-1.5 shrink-0"
                    title="Έλεγχος συμβατότητας & σύνδεσης με το URL για Android APK / Web"
                  >
                    ...
                    <span className="hidden sm:inline">Έλεγχος APK</span>
                  </button>

                  <button
                    type="button"
                    onClick={handleOpenUrlInExternalBrowser}
                    className="p-3.5 rounded-xl bg-[#1f1a14] border border-[#c89b3c]/30 hover:bg-[#c89b3c]/20 text-[#a89984] hover:text-[#e6c670] transition-all shrink-0"
                    title="Άνοιγμα της σελίδας σε εξωτερικό πρόγραμμα περιήγησης (Browser)"
                  >
                    <ExternalLink className="w-4 h-4" />
                  </button>
                </div>

                {/* URL Connection Test Diagnosis Banner */}
                {urlTestStatus && (
                  <div
                    className={`p-3 rounded-xl border text-xs font-serif flex items-start justify-between gap-3 animate-in fade-in transition-all ${
                      urlTestStatus.testing
                        ? "bg-[#1f1a14] border-[#c89b3c]/40 text-[#e6c670]"
                  ...
                  >
                    <div className="flex items-start gap-2">
                      {urlTestStatus.testing ? (
                        <RefreshCw className="w-4 h-4 text-[#c89b3c] animate-spin shrink-0 mt-0.5" />
                      ) : urlTestStatus.success ? (
                      ...
                      <div>
                        <div className="font-semibold">
                          {urlTestStatus.testing
                            ? "Δοκιμή σύνδεσης σε εξέλιξη (Jina AI Reader / Multi-Proxy)..."
                            : urlTestStatus.message}
                        </div>
                        {urlTestStatus.engine && (
                          <div className="text-[11px] opacity-85 mt-0.5">
                            Μηχανή ανάκτησης: <strong>{urlTestStatus.engine}</strong>
                          </div>
                        )}
                      </div>
                    </div>
                    <button
                      type="button"
                      onClick={() => setUrlTestStatus(null)}
                      className="text-[#a89984] hover:text-[#f5ebd7] p-1"
                    >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="π.χ. 666, 888 ή κενό για όλα"
                ...
                <div className="space-y-1.5 pt-1">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {[
                      ...
                      { label: "Σελίδες 100 - 300", start: 100, end: 300 },
                      { label: "Σελίδες 10 - 666", start: 10, end: 666 },
                      { label: "Όλες (1 - 666)", start: 1, end: urlDetectedPages || 666 },
                    ].map((preset, idx) => (
                      <button
                        key={idx}
                        type="button"
      ...
      {activeMode === "batchText" && (
        <div className="space-y-6">
          <div className="p-6 md:p-8 rounded-2xl bg-[#1a1612]/90 border border-[#c89b3c]/20 shadow-xl space-y-6">
            ...
            <div className="space-y-4">
              <SystemSelectionBar
                currentSystem={batchGematriaSystem}
                onChangeSystem={(sys) => setBatchGematriaSystem(sys)}
                showChart={showBatchAlphabetChart}
                onToggleChart={() => setShowBatchAlphabetChart((prev) => !prev)}
                title="Σύστημα Αρίθμησης & Αλφάβητο Κειμένων"
              />
